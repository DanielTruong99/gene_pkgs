...
{       
    ...
    _cfg.observation_dim  = _node->declare_parameter("model.observation_dim", 0);
...
void RLPolicy::_compute_observation(const Inputs& in)
{
    ...
    */
    for(auto w : in.base_ang_vel) _input_obs.push_back(w * 0.25f);

    for(auto g : in.projected_gravity) _input_obs.push_back(g);

    for(size_t index = 0; index < in.q.size(); index++)
        _input_obs.push_back(in.q[index] - _cfg.default_joint_positions[index]);

    for(auto dq : in.dq) _input_obs.push_back(dq * 0.05f);

    for(auto prev_action : _prev_action) _input_obs.push_back(prev_action);
    ...
    for(size_t index = 0; index < in.v_cmds.size(); index++) 
        _input_obs.push_back(in.v_cmds[index] * cmd_scales[index]);

    // std::vector<float> gait_parameters{0, 0, 0, 0,}; //! Test
    for(auto gait_param : in.gait_parameters) _input_obs.push_back(gait_param);
