        ...
        switch (event->signal)
        {
            ...
            case Signal::TIMEOUT_5MS_SIG:
            {
                ...
                */                
                ...
                {
                    float heading_angle = _robot_interface->get_heading_angle();
                    float wz_cmd = -1.1f * heading_angle;
                    wz_cmd = wz_cmd < -0.8f ? -0.8f : wz_cmd < 0.8f ? wz_cmd : 0.8f;
                    obs.v_cmds = std::vector<float>{1.0f, 0.0f, wz_cmd};
                    obs.gait_parameters = std::vector<float>{1.0f, 0.5f, 0.5f, 0.1f};
                    // RCLCPP_INFO(_node->get_logger(), "projected_gravity: %f, %f, %f", heading_angle, obs.projected_gravity[1], obs.projected_gravity[2]);

                }

                // RCLCPP_INFO(_node->get_logger(), "projected_gravity: %f, %f, %f", obs.projected_gravity[0], obs.projected_gravity[1], obs.projected_gravity[2]);
                ...
                {
                    filtered_joint_position_cmds[index] = _alpha * filtered_joint_position_cmds[index] + (1- _alpha) * joint_position_cmds[index];
                }
