...
double max_torque[NUM_ACT] = {60, 60, 80, 100, 60, 60, 80, 100, 28, 28};
// const char*  joint_names[NUM_ELMO + NUM_MY] = {
...
void parameter(void *arg)
{
    ...
    /* Left */
    Kp[5] = 50.0; Kd[5] = 3.0; Ki[5] = 0.0;
    // Hip roll
    Kp[6] = 70.0; Kd[6] = 4.0; Ki[6] = 0.0;
    // Hip pitch
    Kp[7] = 300.0; Kd[7] = 4.0; Ki[7] = 0.0;
    // knee pitch 
    Kp[8] = 120.0; Kd[8] = 1.0; Ki[8] = 0.0;
    // ankle pitch
    Kp[9] = 50.0; Kd[9] = 1.0; Ki[9] = 0.0;
...
void pidControl(int actuator_index, double target_pos, double* error_old)
{
    ...
    if (has_transmission[actuator_index]) {
...
struct MotorCommand
{
    ...
    double angular_velocity[NUM_ELMO + NUM_MY]; // Rad/s
    double torque[NUM_ELMO + NUM_MY];           // Nm
    double kp[NUM_ELMO + NUM_MY];
    double kd[NUM_ELMO + NUM_MY];
    bool is_activated;
};
...
void joint_command_sub(void *arg)
{
    ...
    while (true)
    {
        ...
        for (int index = 0; index < NUM_ELMO + NUM_MY; index++)
        {
            ...
            Kp[index] = motor_cmd->kp[index] == 0 ? Kp[index] : motor_cmd->kp[index];
            Kd[index] = motor_cmd->kd[index] == 0 ? Kd[index] : motor_cmd->kd[index];
        }
...
void control(void *arg)
{
    ...
    const float POS_LIMIT[NUM_ACT][2] = {
        {-0.52, 0.52}, // Hip yaw [-90 90]deg [-PI/4, PI/4]rad
        {-0.52, 0.52}, // Hip roll [-90 90]deg [-PI/4, PI/4]rad
        {-0.95, 0.95}, // Hip pitch [-90 90]deg [-PI/2, PI/2]rad
        {-0.95, 0.52}, // Knee pitch [-90 90]deg [-PI/2, PI/2]rad
        ...
        {-0.3, 1.5}, // Ankle pitch [-90 90]deg [-PI/4, PI/4]rad
        {-0.3, 1.5}, // Ankle pitch [-90 90]deg [-PI/4, PI/4]rad
    };
    ...
    while (run) {
        ...
        if(is_limit != is_limit_prev)
        {
            ...
            if(is_limit)
            {
                ...
                stop_allmotors();  // Stop all motors
                for(int index = 0; index < NUM_ACT; index++)
                {
                    if((posRad[index] < POS_LIMIT[index][0] || posRad[index] > POS_LIMIT[index][1])) rt_printf("Position Limit motor %d \n", index);
        ...
        // Broadcast the controller state to be ready
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    rt_task_create(&EtherCAT_task, "EtherCAT_tasking", 0, 67, 0);
    rt_task_start(&EtherCAT_task, &EtherCAT, NULL);

    rt_task_create(&control_task, "controling", 0, 65, 0);
    ...
    */
   rt_task_create(&joint_state_publisher_task, "joint_state_publisher", 0, 61, 0);
   rt_task_start(&joint_state_publisher_task, &joint_state_publisher, state);

   rt_task_create(&joint_command_subscriber_task, "joint_command_subscriber", 0, 62, 0);
